...
int Widget::loadGLTextures()            // Load Bitmaps And Convert To Textures
{
    ...
    gluBuild2DMipmaps(GL_TEXTURE_2D, 3, image1->size().width(),
                      image1->size().height(),
                      GL_BGRA, GL_UNSIGNED_BYTE, data1);
    ...
    QImage* image3 = new QImage("ground.png");
    ...
    QImage* image5 = new QImage("hoon.png");
    image5->mirror();
    ...
    //data = image->mirrored(false,true).bits();
...
void Widget::paintGL( )
{
    ...
    QImage* image1 = new QImage("opencom.bmp");
    GLubyte *data1 = image1->bits( );
    glGenTextures(1, &texture[0]);                  // Create Three Textures
    image1->mirror();
    // Create MipMapped Texture
